...
class Game():
    def __init__(self):
        ...
        self.green = pygame.Color(255, 0, 0)
    ...
    def refresh_screen(self):
        ...
        pygame.display.flip()
        game.fps_controller.tick(35)

    def show_score(self, choice=1):
        s_font = pygame.font.SysFont('monaco', 24)
        s_surf = s_font.render(
            'Score: {0}'.format(self.score), True, self.green)
        s_surf2 = s_font.render(
            'Hard'.format(self.score), True, self.green)
        s_rect = s_surf.get_rect()
        s_rect2 = s_surf2.get_rect()
        # дефолтный случай отображаем результат слева сверху
        if choice == 1:
            s_rect.midtop = (80, 10)
            s_rect2.midtop = (180, 10)
        # при game_overe отображаем результат по центру
        # под надписью game over
        else:
            s_rect.midtop = (360, 120)
            s_rect2.midtop = (460, 120)
        # рисуем прямоугольник поверх surface
        self.play_surface.blit(s_surf, s_rect)
        self.play_surface.blit(s_surf2, s_rect2)
